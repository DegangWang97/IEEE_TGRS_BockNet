...
See more details in papers:
  [1] D. Wang, L. Zhuang, L. Gao, X. Sun, M. Huang, and A. Plaza,
      “BockNet: Blind-Block Reconstruction Network with a Guard Window
      for Hyperspectral Anomaly Detection,” IEEE Trans. Geosci. Remote Sens.,
      vol. 61, 2023, Art. no. 5531916. DOI: 10.1109/TGRS.2023.3335484
      URL: https://ieeexplore.ieee.org/document/10330742
...
BockNet is distributed under the terms of the GNU General Public License 2.0.
...
from dataset import BockNetData
...
class Trainer(object):
    ...
    def train_epoch(self) -> None:
        ...
        if ((self.epoch + 1) % self.save_freq == 0):
            torch.save(self.model.state_dict(), os.path.join(self.model_path, 'BockNet' + '_' + self.opt.dataset + '_' + str(self.epoch + 1) + '.pkl'))
...
def train_model(opt):
    ...
    prefix = 'BockNet' + '_epoch_' + str(opt.epochs)+ '_learning_rate_' + str(opt.learning_rate) + '_blindspot_' + str(opt.blindspot) + '_gpu_ids_' + str(opt.gpu_ids)
    ...
    loader_train, band = BockNetData(opt)
    net = BockNet(blindspot=opt.blindspot, nch_in=band, nch_out=band, nch_ker=opt.nch_ker).to(device)
...
def predict(opt):
    ...
    model_weights = os.path.join(model_path, 'BockNet' + '_' + opt.dataset + '_' + str(opt.epochs) + '.pkl')
...
def main():    
    ...
    parser.add_argument('--blindspot', default=15, type=int, help='blindspot')
    ...
    parser.add_argument('--dataset', default='Beach', type=str, help='dataset to use')
